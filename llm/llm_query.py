...
def parse_preferences_to_sql(query):
    ...
    # Query ChatGPT with the user query
    response = queryChatGPT3(system_message, query)
    try:
        # Extract the response as a string
        sql_query = response.content.strip()  # Adjust to match your API's response format
...
def recommend_songs(query, db_path=DB_PATH, num_recommendations=10):
    # Parse user preferences and convert them into an SQL query
